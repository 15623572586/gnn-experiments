...
class GraphLearning(nn.Module):
    def __init__(self, batch, n_leads, seq_len, step_len, device='cuda'):
        ...
        self.device = device
        self.gru = nn.GRU(input_size=52, hidden_size=52, num_layers=1, batch_first=True)
        self.key_weight = nn.Parameter(torch.Tensor(batch, 52, n_leads)).to(device)  # 邻接矩阵权重向量
        self.query_weight = nn.Parameter(torch.Tensor(batch, 52, n_leads)).to(device)  # 邻接矩阵权重向量
        self.value_weight = nn.Parameter(torch.Tensor(batch, 52, n_leads)).to(device)  # 邻接矩阵权重向量
