...
from models.gcn_gru import GraphLearning
...
class MTGNN(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None,
                 dropout=0.3, subgraph_size=8, node_dim=40, dilation_exponential=1, conv_channels=32,
                 residual_channels=32, skip_channels=64, end_channels=128, seq_length=1000, in_dim=1, out_dim=12,
        ...
        self.graphlearning = GraphLearning(batch=32, n_leads=12, seq_len=500, step_len=500)
        ...
        self.fc = nn.Sequential(
            nn.Linear(12 * out_dim, 256),  # 将这里改成64试试看
            nn.LeakyReLU(),
            nn.Dropout(p=0.5),
            nn.Linear(256, 128),
            nn.LeakyReLU(),
    ...
    def forward(self, input, idx=None):
        ...
        # seq_len = input.size(3)  # input: (batch, 1, nodes, seq_len)
        # assert seq_len == self.seq_length, 'input sequence length not equal to preset sequence length'
        input = input[:, :, -self.seq_length:]
        adp = torch.mean(self.graphlearning(input), dim=0)
