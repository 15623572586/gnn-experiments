...
from models.gcn_resnet import EcgGCNModel
from models.handler import train, save_model, validation
from models.gft_resnet import EcgModel
from data_loader.cpsc_dataset import ECGCpscDataset
...
def loadData(args, epoch):
    ...
    # train_folds = [i for i in range(1, 10)]
    # test_folds = [10]
...
if __name__ == '__main__':
    ...
    # model = resnet34(input_channels=12, num_classes=5).to(args.device)
    # model = EcgModel(seq_len=args.seq_len, num_classes=args.num_classes).to(args.device)
    # model = STGCN(args, blocks).to(args.device)
    # model = EcgGCNModel(seq_len=args.seq_len, num_classes=args.num_classes, batch_size=args.batch_size, device=args.device).to(args.device)
    ...
    # model = MTGNN(gcn_true=True, buildA_true=True, gcn_depth=2, num_nodes=12, device='cuda', seq_length=args.step_len).to(args.device)
    ...
    if args.resume:
        model_path = os.path.join(result_train_file, '52_stemgnn.pt')
